...
class WelcomeViewController: SplashScreenViewController, SplashScreenViewControllerDelegate {
    ...
    func getHealthAuthorizationRequestStatus() {
        ...
        healthStore.getRequestStatusForAuthorization(toShare: shareTypes, read: readTypes) {
            (authorizationRequestStatus, error) in
            ...
            if let error = error {
                ...
                case .unknown:
                    status = "The authorization request status could not be determined bacause an error occurred."
                case .unnecessary:
